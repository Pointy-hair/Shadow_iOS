...
-(NSMutableArray *)CheckDatesForSpecificDayOfCurrentMonth{
    ...
    NSDateComponents *components = [gregorianCalendar components:NSCalendarUnitYear| NSCalendarUnitMonth| NSCalendarUnitWeekday| NSCalendarUnitDay fromDate:currentDate];
    ...
    NSDateComponents *newcomponents = [gregorianCalendar components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitWeekday | NSCalendarUnitDay fromDate:firstDateOfCurMonth];
    ...
    NSDateComponents *lastcomponents = [gregorianCalendar components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitWeekOfMonth | NSCalendarUnitDay fromDate:templastDateOfCurMonth];
    ...
    do {
        ...
        NSString *dateStr = [dateFormatter stringFromDate:nextDate];
        
        [mutArrDates addObject:dateStr];
...
-(NSMutableArray*)checkDateForSpecificDayOfCurrentYear:(NSMutableArray *)WantedDayArray
{
   ...
   // NSLog(@"weekday : %li",(long)firstDateWeekDay);
    ...
    //setting last date to year
    NSInteger curYear = newcomponents.year;
    
    
    //last date is always 31-12-yyyy
    
    //NSLog(@"current month is %ld",(long)curYear);
    newcomponents.year = curYear ;
    newcomponents.day = 31;
    newcomponents.month = 12;
    
    NSDate * templastDateOfCurYear = [gregorianCalendar dateFromComponents:newcomponents]; // One second before the start of next Year
    
    NSDateComponents *lastcomponents = [gregorianCalendar components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitWeekOfMonth | NSCalendarUnitDay fromDate:templastDateOfCurYear];
    
    [lastcomponents setHour:0];
    [lastcomponents setMinute:0];
    [lastcomponents setSecond:0];
    
    NSDate *lastDateOfCurYear = [gregorianCalendar dateFromComponents:lastcomponents];
    
   // NSLog(@"last date is %@",lastDateOfCurYear);
    ...
    //get wanted weekday date
    int i = 0;
    for(i = 0 ;i < WantedDayArray.count;i++)
    {
        
        NSDate *firstWeekDateOfCurMonth = nil;
        
        
        if ([WantedDayArray[i] intValue]== firstDateWeekDay) {
            
            firstWeekDateOfCurMonth = firstDateOfCurMonth;
        }
        else
        {
            NSInteger day = [WantedDayArray[i] intValue]  - firstDateWeekDay;
        ...
        do {
            ...
            NSDate *date = [gregorianCalendar dateByAddingComponents:dayDifference toDate:firstWeekDateOfCurMonth options:0];
            nextDate = date;
        } while([nextDate compare:lastDateOfCurYear] == NSOrderedAscending || [nextDate compare:lastDateOfCurYear] == NSOrderedSame);
